...
import {
  ...
  sberbankNotNegativeConfirmationStatusCodes,
...
const CheckoutPage: React.FC<IProps> = ({}: IProps) => {
  ...
  const handlePaymentConfirm = async () => {
     ...
     */
    if (
      payment?.gateway !== "mirumee.payments.adyen" &&
      payment?.gateway !== "korolev.payments.sberbank"
    ) {
      const paymentStepLink = steps.find(
        step => step.step === CheckoutStep.Payment
      )?.link;
    ...
    if (
      ...
      sberbankNotNegativeConfirmationStatusCodes.includes(
        querystring.resultCode as string
      )
    ) {
